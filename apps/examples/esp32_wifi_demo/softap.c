...
static void __tizenrt_manual_linkset(const char *msg)
{
	ets_printf("[__tizenrt_manual_linkset] send message \"%s\"\n", msg);
	int ret = mq_send(g_dw_nwevent_mqfd, msg, 3, 42);
	if (ret < 0) {
		ets_printf("[__tizenrt_manual_linkset] send message fail\n");
		return;
	}
}
#endif
...
#define CONFIG_ESP_WIFI_SSID "LAMP_SEOKEY_T_E10ADY99914144"
#define CONFIG_ESP_WIFI_PASSWORD "1111122222"
...
static void wifi_init_softap(void)
{
    ...
    ret = esp_wifi_start();
    if(ret) {
        ets_printf("esp_wifi_start failed, %d\n", ret);
        return;
    }
...
void wifi_softap_entry()
{
	...
	if (g_dw_nwevent_mqfd == (mqd_t)ERROR) {
		ets_printf("[wifi_softap_entry] iotivity connect event message queue init fail");
		return 0;
	}
#endif
