...
if [ ${CONFIG_ENABLE_IOTIVITY} -eq 1 ]; then
	...
	OPTIONS="TARGET_OS=tizenrt TARGET_ARCH=${IOTIVITY_TARGET_ARCH} TARGET_TRANSPORT=IP ROUTING=${IOTIVITY_ROUTING} DISABLE_TCP_SERVER=yes TC_PREFIX=xtensa-esp32-elf- VERBOSE=no TIZENRT_OS_DIR=${TOPDIR} PLATFORM_TLS=${PLATFORM_TLS} ARCH_BOARD=esp32_DevKit -j 8"
