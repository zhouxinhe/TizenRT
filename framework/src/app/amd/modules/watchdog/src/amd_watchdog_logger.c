...
#define LOG_PATH "/run/aul/log/amd_watchdog.log"
#define LOG_MAX_SIZE 131072
#define LOG_MAX_BUF_SIZE 128
#define LOG_MAX_FMT_SIZE 64

struct watchdog_logger_s {
	int fd;
	int index;
};

static struct watchdog_logger_s __logger;
...
int _watchdog_logger_print(const char *tag, const char *format, ...)
{
	...
	if (offset >= LOG_MAX_SIZE)
		offset = lseek(__logger.fd, 0, SEEK_SET);

	va_start(ap, format);
	vsnprintf(format_buf, sizeof(format_buf), format, ap);
	va_end(ap);

	snprintf(buf, sizeof(buf), "[%6d] %-10s %-64s %s",
